...
const ActivityRings: React.FC<ActivityRingsProps> = ({data, size}) => {
  ...
  useEffect(() => {
    moveProgress.value = withDelay(
      200,
      withSpring(data.move.current / data.move.goal),
    );
    exerciseProgress.value = withDelay(
      400,
      withSpring(data.exercise.current / data.exercise.goal),
    );
    standProgress.value = withDelay(
      600,
      withSpring(data.stand.current / data.stand.goal),
    );
  }, [data]);

  const moveAnimatedStyle = useAnimatedStyle(() => ({
    strokeDashoffset:
      moveCircumference * (1 - Math.min(moveProgress.value, 1)),
  }));

  const exerciseAnimatedStyle = useAnimatedStyle(() => ({
    strokeDashoffset:
      exerciseCircumference * (1 - Math.min(exerciseProgress.value, 1)),
  }));

  const standAnimatedStyle = useAnimatedStyle(() => ({
    strokeDashoffset:
      standCircumference * (1 - Math.min(standProgress.value, 1)),
  }));
